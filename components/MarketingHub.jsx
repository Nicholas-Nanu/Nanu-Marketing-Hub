...
const PROJECT_STATUS_COLORS: any = { Planning:"#748FFC", Active:"#69DB7C", Paused:"#FFA94D", Complete:"#4E6A78" };
...
const OUTREACH_STATUS_COLORS: any = { Identified:"#748FFC", Contacted:"#FFA94D", "In Conversation":"#DA77F2", Confirmed:"#69DB7C", Declined:"#FF6B6B", Complete:"#4E6A78" };
...
function exportDOCX(title: string, items: {heading:string, body:string}[], filename: string) {
...
export default function MarketingHub() {
  ...
  const [weeklyThemes, setWeeklyThemes] = useState(INIT_WEEKLY_THEMES);
  ...
  const [keyDates, setKeyDates] = useState(INIT_KEY_DATES);
  const [campaigns, setCampaigns] = useState(INIT_CAMPAIGNS);
  const [activity, setActivity] = useState(INIT_ACTIVITY);
  const [projects, setProjects] = useState(INIT_PROJECTS);
  const [outreach, setOutreach] = useState(INIT_OUTREACH);
  const [outreachFilter, setOutreachFilter] = useState("All");

  const theme = getTheme(dark);
  const isAdmin = curUser?.role === "Admin";
  const uName = (id: string) => users.find(u=>u.id===id)?.name || "Unknown";
  const log = (action: string, target: string, sec: string) => { if(!curUser) return; setActivity((p: any[])=>[{id:uid("a"),user:curUser.id,action,target,section:sec,time:new Date().toISOString()},...p].slice(0,50)); };
  const openM = (type: string, data: any = {}) => { setForm(data); setModal(type); };
  ...
  const renderSection = () => {
          ...
          {users.map(u=>(
            <Card key={u.id} theme={theme} style={{ position:"relative" }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:6, marginTop:8, flexWrap:"wrap" }}>
                ...
                {u.socials && Object.entries(u.socials).filter(([_,v])=>v).map(([k,v]:any)=>(
      ...
      <div>
        <SectionHead theme={theme} right={<>
          ...
          <Btn theme={theme} small onClick={()=>{
            const myTasks = taskView==="mine" ? sortedTasks : sortedTasks.filter((t:any)=>t.owner===curUser.id);
            const rows = [["Title","Status","Priority","Due Date","Owner","Project","Blocker","Notes"]];
            myTasks.forEach((t:any)=>rows.push([t.title,t.status,t.priority||"",t.dueDate,uName(t.owner),projects.find((p:any)=>p.id===t.project)?.name||"",t.blocker||"",t.notes||""]));
            exportCSV(rows,`nanu-tasks-${new Date().toISOString().slice(0,10)}.csv`);
          }}><Download size={13}/> CSV</Btn>
          <Btn theme={theme} small onClick={()=>{
            const myTasks = taskView==="mine" ? sortedTasks : sortedTasks.filter((t:any)=>t.owner===curUser.id);
            const items = myTasks.map((t:any)=>({
              heading:`${t.title} — ${t.status}${t.priority?" · "+t.priority:""}`,
              body:`Owner: ${uName(t.owner)}\nDue: ${t.dueDate}\n${t.project?`Project: ${projects.find((p:any)=>p.id===t.project)?.name||""}\n`:""}${t.blocker?`Blocker: ${t.blocker}\n`:""}${t.notes?`\n${t.notes}`:""}`
            }));
        ...
        {sortedTasks.map((t:any)=>{
          ...
          return <Card key={t.id} theme={theme} onClick={()=>openM("editTask",{...t})} style={{padding:12,marginBottom:6,cursor:"pointer",borderLeft:`3px solid ${TASK_STATUS_COLORS[t.status]||theme.border}`,background:hl?`${TASK_STATUS_COLORS[t.status]}06`:theme.bgCard}}>
            ...
            {t.linkedContent&&calendar.find((c:any)=>c.id===t.linkedContent)&&<div style={{marginTop:4,display:"flex",alignItems:"center",gap:4}}><Link2 size={11} color={theme.teal}/><span style={{fontSize:11,color:theme.teal}}>Linked: {calendar.find((c:any)=>c.id===t.linkedContent)?.title}</span></div>}
          </Card>;
        })}
      </div>
    );
      ...
      <div>
        ...
        <div className="nanu-grid-2col">
          {projects.map((proj:any)=>{
            const projTasks = tasks.filter((t:any)=>t.project===proj.id);
            ...
            return <Card key={proj.id} theme={theme} style={{position:"relative"}}>
              ...
              <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:4}}>
                {projTasks.filter((t:any)=>t.status!=="Done").slice(0,4).map((t:any)=>(
                ...
                {projTasks.filter((t:any)=>t.status!=="Done").length > 4 && <div style={{fontSize:11,color:theme.textMut,paddingLeft:8}}>+{projTasks.filter((t:any)=>t.status!=="Done").length - 4} more</div>}
              </div>
    ...
    case "outreach": return (
      <div>
        <SectionHead theme={theme} right={<>
          <Sel theme={theme} options={[{value:"All",label:"All Types"},...OUTREACH_TYPES.map(t=>({value:t,label:t}))]} value={outreachFilter} onChange={(e:any)=>setOutreachFilter(e.target.value)} style={{width:"auto",fontSize:13,padding:"6px 10px"}}/>
        ...
        <div className="nanu-grid-summary" style={{marginBottom:18}}>
          {OUTREACH_TYPES.map(type=>{
            const count = outreach.filter((o:any)=>o.type===type).length;
            const confirmed = outreach.filter((o:any)=>o.type===type && o.status==="Confirmed").length;
        ...
        <div className="nanu-kanban">
          {OUTREACH_STATUSES.map(status=>{
            const items = outreach.filter((o:any)=>(outreachFilter==="All"||o.type===outreachFilter)&&o.status===status);
        ...
        <div className="nanu-grid-2col">
            ...
            {users.map(u=>(
              <div key={u.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 0",borderTop:`1px solid ${theme.borderLight}`}}>
                ...
                <div style={{display:"flex",gap:6}}>
                  ...
                  {u.id!==curUser.id&&<Btn theme={theme} small danger onClick={()=>{if(confirm(`Remove ${u.name}?`)){setUsers(p=>p.filter(x=>x.id!==u.id));log("removed",u.name,"Admin")}}}><Trash2 size={12}/></Btn>}
                </div>
              </div>
            ))}
          </Card>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <Card theme={theme}>
              ...
              <div style={{fontSize:14,color:theme.textSec,lineHeight:2}}>Team: {users.length} · Calendar: {calendar.length} · Tasks: {tasks.filter((t:any)=>t.status!=="Done").length} · Projects: {projects.length} · Outreach: {outreach.length} · Notes: {notes.length}</div>
            </Card>
          </div>
        </div>
      </div>
  ...
  const renderModal = () => {
    ...
    switch(modal) {
      ...
      case "editTask": return <Modal theme={theme} title={form.id?"Edit Task":"New Task"} onClose={closeM} width={580}><F>
        ...
        <Row><div><Label theme={theme}>Project</Label><Sel theme={theme} options={[{value:"",label:"None"},...projects.map((p:any)=>({value:p.id,label:p.name}))]} value={form.project||""} onChange={(e:any)=>setForm(p=>({...p,project:e.target.value}))}/></div><div><Label theme={theme}>Linked Content</Label><Sel theme={theme} options={[{value:"",label:"None"},...calendar.map((c:any)=>({value:c.id,label:`${c.title} (${c.platform})`}))]} value={form.linkedContent||""} onChange={(e:any)=>setForm(p=>({...p,linkedContent:e.target.value}))}/></div></Row>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {[["linkedin","LinkedIn"],["x","X / Twitter"],["instagram","Instagram"],["tiktok","TikTok"],["youtube","YouTube"]].map(([key,label])=>(
              <div key={key} style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <Input theme={theme} value={form.socials?.[key]||""} onChange={(e:any)=>setForm((p:any)=>({...p,socials:{...(p.socials||{}), [key]:e.target.value}}))} placeholder={`https://...`} style={{fontSize:13}}/>
              </div>
            ))}
          </div>
        </div>
        <Actions onSave={()=>{if(form.id){setUsers((p:any)=>p.map((u:any)=>u.id===form.id?{...form,socials:form.socials||{}}:u));log("updated",form.name,"Team")}else{setUsers((p:any)=>[...p,{...form,id:uid("u"),socials:form.socials||{}}]);log("added",form.name,"Team")}closeM()}}/>
      </F></Modal>;
      ...
      case "editThemes": return <Modal theme={theme} title="Edit Weekly Themes" onClose={closeM} width={600}>
        ...
        <div style={{display:"flex",justifyContent:"flex-end",marginTop:16,gap:10}}><Btn theme={theme} onClick={closeM}>Cancel</Btn><Btn primary theme={theme} onClick={()=>{log("updated","Weekly Themes","Admin");closeM()}}>Save</Btn></div>
      </Modal>;
      ...
      case "editGrowth": return <Modal theme={theme} title="Update Nanu User Growth" onClose={closeM} width={600}>
        ...
        <div style={{display:"flex",justifyContent:"flex-end",gap:10}}>
          ...
          <Btn primary theme={theme} onClick={()=>{
            setStats(p=>({...p,weeklyGrowth:form.entries||[],lastUpdated:new Date().toISOString().split("T")[0]}));
      ...
      case "editProject": return <Modal theme={theme} title={form.id?"Edit Project":"New Project"} onClose={closeM} width={580}><F>
        <div><Label theme={theme}>Project Name</Label><Input theme={theme} value={form.name||""} onChange={(e:any)=>setForm((p:any)=>({...p,name:e.target.value}))}/></div>
        <div><Label theme={theme}>Description</Label><Textarea theme={theme} value={form.description||""} onChange={(e:any)=>setForm((p:any)=>({...p,description:e.target.value}))}/></div>
        <Row><div><Label theme={theme}>Owner</Label><Sel theme={theme} options={users.map((u:any)=>({value:u.id,label:u.name}))} value={form.owner||""} onChange={(e:any)=>setForm((p:any)=>({...p,owner:e.target.value}))}/></div><div><Label theme={theme}>Status</Label><Sel theme={theme} options={PROJECT_STATUSES} value={form.status||"Planning"} onChange={(e:any)=>setForm((p:any)=>({...p,status:e.target.value}))}/></div></Row>
        <div><Label theme={theme}>Colour</Label><input type="color" value={form.color||theme.teal} onChange={(e:any)=>setForm((p:any)=>({...p,color:e.target.value}))} style={{width:48,height:36,border:"none",borderRadius:8,cursor:"pointer"}}/></div>
        <Actions onDelete={form.id?()=>{setProjects((p:any)=>p.filter((x:any)=>x.id!==form.id));log("deleted",form.name,"Projects");closeM()}:null} onSave={()=>{
          if(form.id){setProjects((p:any)=>p.map((x:any)=>x.id===form.id?{...form}:x));log("updated",form.name,"Projects")}
          else{setProjects((p:any)=>[...p,{...form,id:uid("proj")}]);log("created",form.name,"Projects")}
          closeM();
        }}/>
      </F></Modal>;
      ...
      case "editOutreach": return <Modal theme={theme} title={form.id?"Edit Outreach Contact":"New Outreach Contact"} onClose={closeM} width={580}><F>
        <div><Label theme={theme}>Name</Label><Input theme={theme} value={form.name||""} onChange={(e:any)=>setForm((p:any)=>({...p,name:e.target.value}))}/></div>
        <Row><div><Label theme={theme}>Type</Label><Sel theme={theme} options={OUTREACH_TYPES} value={form.type||"Community"} onChange={(e:any)=>setForm((p:any)=>({...p,type:e.target.value}))}/></div><div><Label theme={theme}>Status</Label><Sel theme={theme} options={OUTREACH_STATUSES} value={form.status||"Identified"} onChange={(e:any)=>setForm((p:any)=>({...p,status:e.target.value}))}/></div></Row>
        <Row><div><Label theme={theme}>Platform / Channel</Label><Input theme={theme} value={form.platform||""} onChange={(e:any)=>setForm((p:any)=>({...p,platform:e.target.value}))} placeholder="e.g. Podcast, Discord, YouTube"/></div><div><Label theme={theme}>Owner</Label><Sel theme={theme} options={users.map((u:any)=>({value:u.id,label:u.name}))} value={form.owner||""} onChange={(e:any)=>setForm((p:any)=>({...p,owner:e.target.value}))}/></div></Row>
        <Row><div><Label theme={theme}>Date</Label><Input theme={theme} type="date" value={form.date||""} onChange={(e:any)=>setForm((p:any)=>({...p,date:e.target.value}))}/></div><div><Label theme={theme}>URL</Label><Input theme={theme} value={form.url||""} onChange={(e:any)=>setForm((p:any)=>({...p,url:e.target.value}))} placeholder="https://..."/></div></Row>
        <div><Label theme={theme}>Notes</Label><Textarea theme={theme} value={form.notes||""} onChange={(e:any)=>setForm((p:any)=>({...p,notes:e.target.value}))} placeholder="Context, talking points, follow-up actions..."/></div>
        <Actions onDelete={form.id?()=>{setOutreach((p:any)=>p.filter((x:any)=>x.id!==form.id));log("deleted",form.name,"Outreach");closeM()}:null} onSave={()=>{
          if(form.id){setOutreach((p:any)=>p.map((x:any)=>x.id===form.id?{...form}:x));log("updated",form.name,"Outreach")}
          else{setOutreach((p:any)=>[...p,{...form,id:uid("out")}]);log("created",form.name,"Outreach")}
          closeM();
        }}/>
      </F></Modal>;
      ...
      case "editSocials": return <Modal theme={theme} title="Edit My Socials" onClose={closeM}><F>
        ...
        {[["linkedin","LinkedIn"],["x","X / Twitter"],["instagram","Instagram"],["tiktok","TikTok"],["youtube","YouTube"]].map(([key,label])=>(
          <div key={key} style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
            <span style={{fontSize:12,color:theme.textSec,width:80,flexShrink:0}}>{label}</span>
            <Input theme={theme} value={form.socials?.[key]||""} onChange={(e:any)=>setForm((p:any)=>({...p,socials:{...(p.socials||{}), [key]:e.target.value}}))} placeholder={`https://...`}/>
          </div>
        ))}
        <Actions onSave={()=>{
          setUsers((p:any)=>p.map((u:any)=>u.id===curUser.id?{...u,socials:form.socials||{}}:u));
