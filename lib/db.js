...
export async function loadAllData() {
  ...
  return {
    ...
    meetingActions: get(53).map(a => ({ id: a.id, meetingId: a.meeting_id || '', text: a.text, owner: a.owner || '', ownerText: a.owner_text || '', dueDate: a.due_date || '', status: a.status || 'Open', taskId: a.task_id || '', notes: a.notes || '' })),
  };
}
...
export const saveMeetingAction = (a) => run(() => supabase.from('meeting_actions').upsert({
  ...
  owner_text: a.ownerText||'', due_date: a.dueDate||'', status: a.status||'Open',
