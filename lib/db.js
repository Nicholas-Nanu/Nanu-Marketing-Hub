...
export async function loadAllData() {
  const results = await Promise.allSettled([
    ...
    supabase.from('media_guests').select('*'),
    supabase.from('phase_actions').select('*').order('seq'),
    supabase.from('roadmap_items').select('*').order('sort_order'),
    supabase.from('operating_structures').select('*'),
  ...
  return {
    ...
    tasks: get(3).map(t => ({ id: t.id, title: t.title, owners: t.owners || [], dueDate: t.due_date, status: t.status, blocker: t.blocker, priority: t.priority, notes: t.notes, linkedContent: t.linked_content, project: t.project, updates: t.updates || [], createdBy: t.created_by || '', createdDate: t.created_date || '', context: t.context || '', contactName: t.contact_name || '', contactDetail: t.contact_detail || '', outcome: t.outcome || '', blockerOwner: t.blocker_owner || '', blockerSince: t.blocker_since || '', blockerEscalated: !!t.blocker_escalated })),
    ...
    mediaProducts: get(41).map(p => ({ id: p.id, name: p.name, description: p.description || '', format: p.format || '', cadence: p.cadence || '', showrunner: p.showrunner || '', status: p.status || 'Active', driveUrl: p.drive_url || '', color: p.color || '#1FC2C2', sortOrder: p.sort_order || 0, notes: p.notes || '', episodePrefix: p.episode_prefix || '', defaultOwner: p.default_owner || '', checklistTemplate: p.checklist_template || [], publishDay: p.publish_day || '' })),
    mediaItems: get(42).map(i => ({ id: i.id, productId: i.product_id || '', title: i.title, stage: i.stage || 'Idea', owner: i.owner || '', episodeNo: i.episode_no || '', summary: i.summary || '', dueDate: i.due_date || '', airDate: i.air_date || '', scriptUrl: i.script_url || '', assetsUrl: i.assets_url || '', finalUrl: i.final_url || '', blocker: i.blocker || '', notes: i.notes || '', needsDesign: !!i.needs_design, designStatus: i.design_status || '', stageSince: i.stage_since || '', checklist: i.checklist || [], linkedTasks: i.linked_tasks || [], blockerOwner: i.blocker_owner || '', blockerSince: i.blocker_since || '', blockerEscalated: !!i.blocker_escalated })),
    ...
    mediaGuests: get(48).map(g => ({ id: g.id, name: g.name, email: g.email || '', phone: g.phone || '', productId: g.product_id || '', itemId: g.item_id || '', bio: g.bio || '', recordedDate: g.recorded_date || '', releaseSigned: !!g.release_signed, releaseUrl: g.release_url || '', status: g.status || 'Approached', notes: g.notes || '' })),
    phaseActions: get(49).map(a => ({ id: a.id, seq: a.seq || 0, action: a.action, detail: a.detail || '', ownerUser: a.owner_user || '', ownerText: a.owner_text || '', dueLabel: a.due_label || '', dueDate: a.due_date || '', status: a.status || 'Not started', notes: a.notes || '' })),
    roadmapItems: get(50).map(r => ({ id: r.id, title: r.title, description: r.description || '', area: r.area || '', bucket: r.bucket || 'Requested', owner: r.owner || '', priority: r.priority || 'Medium', effort: r.effort || '', progress: r.progress || 0, targetQuarter: r.target_quarter || '', targetDate: r.target_date || '', shippedDate: r.shipped_date || '', releaseTag: r.release_tag || '', requestedBy: r.requested_by || '', requestedDate: r.requested_date || '', why: r.why || '', decisionNote: r.decision_note || '', linkedTasks: r.linked_tasks || [], sortOrder: r.sort_order || 0, notes: r.notes || '', blocker: r.blocker || '', blockerOwner: r.blocker_owner || '', blockerSince: r.blocker_since || '', blockerEscalated: !!r.blocker_escalated })),
    opStructures: get(51).map(s => ({ userId: s.user_id, subtitle: s.subtitle || '', intro: s.intro || '', owns: s.owns || [], shared: s.shared || [], cadence: s.cadence || [], standing: s.standing || [], focus: s.focus || [], sourceNote: s.source_note || '' })),
    meetings: get(52).map(m => ({ id: m.id, title: m.title, date: m.date || '', type: m.type || 'Team', attendees: m.attendees || [], summary: m.summary || '', decisions: m.decisions || '', recordingUrl: m.recording_url || '', source: m.source || '', notes: m.notes || '', createdBy: m.created_by || '' })),
    meetingActions: get(53).map(a => ({ id: a.id, meetingId: a.meeting_id || '', text: a.text, owner: a.owner || '', ownerText: a.owner_text || '', dueDate: a.due_date || '', status: a.status || 'Open', priority: a.priority || 'Medium', taskId: a.task_id || '', notes: a.notes || '', blocker: a.blocker || '', blockerOwner: a.blocker_owner || '', blockerSince: a.blocker_since || '', blockerEscalated: !!a.blocker_escalated })),
  };
}
...
export const saveTask = (t) => run(() => supabase.from('tasks').upsert({ id: t.id, title: t.title, owners: t.owners || [], due_date: t.dueDate, status: t.status, blocker: t.blocker, priority: t.priority, notes: t.notes, linked_content: t.linkedContent, project: t.project || null, updates: t.updates || [], created_by: t.createdBy || null, created_date: t.createdDate || '', context: t.context || '', contact_name: t.contactName || '', contact_detail: t.contactDetail || '', outcome: t.outcome || '', blocker_owner: t.blockerOwner || null, blocker_since: t.blockerSince || '', blocker_escalated: !!t.blockerEscalated }));
...
export const saveMediaProduct = (p) => run(() => supabase.from('media_products').upsert({
  ...
  episode_prefix: p.episodePrefix||'', default_owner: p.defaultOwner||null,
  checklist_template: p.checklistTemplate||[], publish_day: p.publishDay||''
}));
...
export const saveMediaItem = (i) => run(() => supabase.from('media_items').upsert({
  ...
  needs_design: !!i.needsDesign, design_status: i.designStatus||'', stage_since: i.stageSince||'',
  checklist: i.checklist||[], linked_tasks: i.linkedTasks||[],
  blocker_owner: i.blockerOwner||null, blocker_since: i.blockerSince||'', blocker_escalated: !!i.blockerEscalated
}));
...
export const saveMediaGuest = (g) => run(() => supabase.from('media_guests').upsert({
  id: g.id, name: g.name, email: g.email||'', phone: g.phone||'', product_id: g.productId||'',
  item_id: g.itemId||'', bio: g.bio||'', recorded_date: g.recordedDate||'',
  release_signed: !!g.releaseSigned, release_url: g.releaseUrl||'', status: g.status||'Approached', notes: g.notes||''
}));
export const deleteMediaGuest = (id) => run(() => supabase.from('media_guests').delete().eq('id', id));

export const savePhaseAction = (a) => run(() => supabase.from('phase_actions').upsert({
  id: a.id, seq: a.seq||0, action: a.action, detail: a.detail||'', owner_user: a.ownerUser||null,
  owner_text: a.ownerText||'', due_label: a.dueLabel||'', due_date: a.dueDate||'',
  status: a.status||'Not started', notes: a.notes||''
}));
export const deletePhaseAction = (id) => run(() => supabase.from('phase_actions').delete().eq('id', id));

// --- Roadmap ---
export const saveRoadmapItem = (r) => run(() => supabase.from('roadmap_items').upsert({
  id: r.id, title: r.title, description: r.description||'', area: r.area||'',
  bucket: r.bucket||'Requested', owner: r.owner||null, priority: r.priority||'Medium',
  effort: r.effort||'', progress: r.progress||0, target_quarter: r.targetQuarter||'',
  target_date: r.targetDate||'', shipped_date: r.shippedDate||'', release_tag: r.releaseTag||'',
  requested_by: r.requestedBy||null, requested_date: r.requestedDate||'', why: r.why||'',
  decision_note: r.decisionNote||'', linked_tasks: r.linkedTasks||[], sort_order: r.sortOrder||0, notes: r.notes||'',
  blocker: r.blocker||'', blocker_owner: r.blockerOwner||null, blocker_since: r.blockerSince||'', blocker_escalated: !!r.blockerEscalated
}));
export const deleteRoadmapItem = (id) => run(() => supabase.from('roadmap_items').delete().eq('id', id));

// --- Operating structures (per-person roles & weekly rhythm) ---
export const saveOpStructure = (s) => run(() => supabase.from('operating_structures').upsert({
  user_id: s.userId, subtitle: s.subtitle||'', intro: s.intro||'',
  owns: s.owns||[], shared: s.shared||[], cadence: s.cadence||[],
  standing: s.standing||[], focus: s.focus||[], source_note: s.sourceNote||'',
  updated_at: new Date().toISOString()
}));
export const deleteOpStructure = (userId) => run(() => supabase.from('operating_structures').delete().eq('user_id', userId));

// --- Meetings & action points ---
export const saveMeeting = (m) => run(() => supabase.from('meetings').upsert({
  id: m.id, title: m.title, date: m.date||'', type: m.type||'Team', attendees: m.attendees||[],
  summary: m.summary||'', decisions: m.decisions||'', recording_url: m.recordingUrl||'',
  source: m.source||'', notes: m.notes||'', created_by: m.createdBy||null
}));
export const deleteMeeting = (id) => run(() => supabase.from('meetings').delete().eq('id', id));

export const saveMeetingAction = (a) => run(() => supabase.from('meeting_actions').upsert({
  id: a.id, meeting_id: a.meetingId||null, text: a.text, owner: a.owner||null,
  owner_text: a.ownerText||'', due_date: a.dueDate||'', status: a.status||'Open', priority: a.priority||'Medium',
  task_id: a.taskId||'', notes: a.notes||'',
  blocker: a.blocker||'', blocker_owner: a.blockerOwner||null, blocker_since: a.blockerSince||'', blocker_escalated: !!a.blockerEscalated
}));
export const deleteMeetingAction = (id) => run(() => supabase.from('meeting_actions').delete().eq('id', id));
