...
export async function loadAllData() {
  ...
  return {
    ...
    tasks: get(3).map(t => ({ id: t.id, title: t.title, owners: t.owners || [], dueDate: t.due_date, status: t.status, blocker: t.blocker, priority: t.priority, notes: t.notes, linkedContent: t.linked_content, project: t.project })),
    ...
    projects: get(17).map(p => ({ id: p.id, name: p.name, description: p.description, color: p.color, owner: p.owner, status: p.status, members: p.members || [], notes: p.notes || '', links: p.links || [] })),
    outreach: get(18).map(o => ({ id: o.id, name: o.name, type: o.type, platform: o.platform, status: o.status, owner: o.owner, notes: o.notes, url: o.url, date: o.date, contact_name: o.contactName || '', contact_email: o.contactEmail || '' })),
...
export const saveTask = (t) => run(() => supabase.from('tasks').upsert({ id: t.id, title: t.title, owners: t.owners || [], due_date: t.dueDate, status: t.status, blocker: t.blocker, priority: t.priority, notes: t.notes, linked_content: t.linkedContent, project: t.project || null }));
...
export const saveProject = (p) => run(() => supabase.from('projects').upsert({ id: p.id, name: p.name, description: p.description, color: p.color, owner: p.owner, status: p.status, members: p.members || [], notes: p.notes || '', links: p.links || [] }));
export const deleteProject = (id) => run(() => supabase.from('projects').delete().eq('id', id));
export const saveOutreach = (o) => run(() => supabase.from('outreach').upsert({ id: o.id, name: o.name, type: o.type, platform: o.platform, status: o.status, owner: o.owner, notes: o.notes, url: o.url, date: o.date, contact_name: o.contactName || '', contact_email: o.contactEmail || '' }));
