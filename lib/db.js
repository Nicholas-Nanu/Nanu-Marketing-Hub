...
export async function loadAllData() {
  ...
  return {
    ...
    stats: {
      ...
      platforms: Object.fromEntries((platformItems || []).map(p => [p.platform, { followers: p.followers, reach: p.reach, engagement: Number(p.engagement), growth: Number(p.growth) }])),
...
export const savePlatformStat = (platform, data) =>
  supabase.from('platform_stats').upsert({ platform, followers: data.followers, reach: data.reach, engagement: data.engagement, growth: data.growth });
