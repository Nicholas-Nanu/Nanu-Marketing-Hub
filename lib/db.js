...
export async function loadAllData() {
  ...
  return {
    ...
    projects: get(17).map(p => ({ id: p.id, name: p.name, description: p.description, color: p.color, owner: p.owner, status: p.status, members: p.members || [] })),
...
export const saveProject = (p) => run(() => supabase.from('projects').upsert({ id: p.id, name: p.name, description: p.description, color: p.color, owner: p.owner, status: p.status, members: p.members || [] }));
