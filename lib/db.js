...
// --- Chat Messages ---
export const saveChatMessages = (messages) => run(() => supabase.from('chat_data').upsert({ id: 'messages', data: messages }));
export const loadChatMessages = async () => {
  const res = await run(() => supabase.from('chat_data').select('data').eq('id', 'messages').single());
  return res?.data?.data || {};
};

// --- Chat Last Read ---
export const saveChatLastRead = (userId, lastRead) => run(() => supabase.from('chat_last_read').upsert({ user_id: userId, last_read: lastRead }));
export const loadChatLastRead = async (userId) => {
  const res = await run(() => supabase.from('chat_last_read').select('last_read').eq('user_id', userId).single());
  return res?.data?.last_read || {};
};
